...
- **Retirement planning sub-page** at `/portfolio/retirement` — full projection (years remaining, current value, real + nominal target, required CAGR, on-track/tight/etc. verdict) plus the spending-sustainability sub-section (sustainable monthly, withdrawal rate, years portfolio lasts, perpetual/comfortable/etc. verdict) plus citations and caveats. `/portfolio` shows a compact preview tile with the headline numbers and a button to the full page.
...
| Retirement planning (years remaining, required CAGR, spending sustainability, verdict) | `/portfolio/retirement` |
...
app/
  ...
  account_classifier.rb       # Broker account name → tax kind (taxable / Roth / 401k / IRA / annuity / HSA)
  expense_ratio_map.rb        # Curated symbol → expense-ratio table (~70 funds + 401k institutional CUSIPs)
